...
Evidence:
- `main.js:98-108` — `handleSectionKey` parses `e.key`, `e.preventDefault()`, calls `navigateToSection`.
- `main.js:288` — `window.addEventListener('keydown', handleSectionKey)`.
- The HUD legend (index.html:153-166) already holds the power LED, sig-path, REV mark, the live scope readout, and the one-time probe hint (`hud-probe-hint`, styled at scroll.css:273-298) — the natural home for a permanent key hint.
...
```html
<!-- index.html — inside .hud-legend, after the probe hint (line ~166) -->
<span class="hud-keyhint" aria-hidden="true">KEYS&nbsp;1–6&nbsp;·&nbsp;JUMP</span>
```

```css
/* scroll.css — near the .hud-probe-hint rules (~line 273) */
.hud-keyhint {
    font-size: 10px;
...
- **Small viewports**: the HUD legend already hides `hud-probe-hint` under `@media (max-width: 900px)` (scroll.css:302-306) — add the keyhint to that same hide rule.
- **Reduced motion**: it's static text — no motion to gate. No `prefers-reduced-motion` change needed.

## Repo conventions to follow

- Hint chrome exemplar: `.hud-probe-hint` (scroll.css:273-298) — same size/letter-spacing family, muted silkscreen color (`rgba(157, 180, 163, …)` is the `--silkscreen-muted` family used throughout).
...
3. Add `body.lite-mode .hud-keyhint { display: none; }` near the other lite-mode rules (~line 1248).
4. In the `@media (max-width: 900px)` block that hides `.hud-probe-hint` (scroll.css:302-306), add `.hud-keyhint` to the same rule.
...
- **Done when**: the keyhint renders in the HUD legend, hides on small viewports and lite mode, and keys 1–6 visibly jump sections.
