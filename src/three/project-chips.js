...
/** @typedef {{ mat: THREE.MeshStandardMaterial, seed: number, gem: THREE.MeshStandardMaterial | null }} FlickerLed */
 ...
 * gemMat is the socket gem's material (category-colored infinity stone;
 * null for chips without one) — the click-to-focus flash (journey.js)
 * brightens it in sync with the LED.
 * @typedef {{ pos: THREE.Vector3, data: any, ledMat: THREE.MeshStandardMaterial, ledBase: number, gemMat: THREE.MeshStandardMaterial | null, mats: THREE.MeshStandardMaterial[] }} ChipRecord
 */
...
/** @typedef {{ mat: THREE.MeshStandardMaterial, seed: number, base: number, gem: THREE.MeshStandardMaterial | null }} SteadyLed */
/** @type {SteadyLed[]} */
const steadyLeds = [];

// ─── Infinity-stone gems ───────────────────────────────────────
// Every project chip carries a small gem in its socket, colored by the
// project's CATEGORY — the same three hues the datasheet filter pins use
// (AI/ML rose, FULL-STACK cyan, SYSTEMS blue), so the board and the filter
// agree on what a category looks like. The finish carries the state:
//   shipped  → polished, faceted octahedron, steady glow (rides the
//              shipped LED's slow breathe, same driver value)
//   building → rough, un-faceted icosahedron, flickering glow (rides the
//              breadboard LED's flicker, same driver value)
// The gem shares the LED's per-frame driver so dim (filter), flicker,
// breathe, and the click-to-focus flash all stay in sync automatically.
/** @type {Record<string, number>} */
const GEM_CATEGORY_COLOR = {
    'AI/ML': 0xf43f5e,      // rose — the AI accelerator bank
    'FULL-STACK': 0x22d3ee, // cyan — the web/interface bus
    'SYSTEMS': 0x3b82f6     // blue — the embedded systems rail
};
const GEM_SOLDERED_EMISSIVE = 1.6;  // steady base (LED base is 1.4)
const GEM_BUILDING_EMISSIVE = 0.8;  // flicker center (LED is 0.7)
...
export function createProjectChips(boardGroup) {
    ...
    // The socket gem per chip — built from the project's category color and
    // finish (shipped = polished, building = rough). Null when the category
    // has no mapped color (never — all three filter categories are mapped).
    /** @param {string} category @param {boolean} building
     *  @returns {THREE.MeshStandardMaterial | null} */
    const makeGemMat = (category, building) => {
        const color = GEM_CATEGORY_COLOR[category];
        if (!color) return null;
        return new THREE.MeshStandardMaterial({
            color,
            emissive: color,
            emissiveIntensity: building ? GEM_BUILDING_EMISSIVE : GEM_SOLDERED_EMISSIVE,
            roughness: building ? 0.95 : 0.12,
            metalness: building ? 0.05 : 0.7,
            flatShading: true // facets catch the bench light
        });
    };
    /** @param {THREE.MeshStandardMaterial} gemMat @param {boolean} building
     *  @returns {THREE.Mesh} */
    const makeGemMesh = (gemMat, building) => {
        // Octahedron = cut gem facets (polished); low-detail icosahedron =
        // rough un-cut nodule (breadboard).
        const geo = building
        ...
        mesh.rotation.set(building ? 0.4 : 0, 0, building ? 0.25 : Math.PI / 4);
        return mesh;
    };
    ...
    projects.forEach((proj, i) => {
        ...
        // Socket gem — category-colored infinity stone. The gem material is
        // created first and threaded into the builders (it also registers
        // the gem on the shared LED driver so both glow in lockstep).
        const gemMat = makeGemMat(proj.category || '', isBuilding);
        const gemMesh = gemMat ? makeGemMesh(gemMat, isBuilding) : null;

        const ledMat = isBuilding
            ? buildBreadboardPatch(group, signal, gemMat, gemMesh)
            : buildSolderedChip(group, chipMat, goldMat, solderTraceMat, busY - rowY, signal, gemMat, gemMesh);
        ...
        const chipMats = [];
        group.traverse((o) => {
            // goldMat excluded so a neighbor's pads never dim; gemMat excluded
            // because the per-frame LED/gem driver (updateProjectChips) owns
            // its emissive — the filter pins it flat via dimmedLedMats instead
            // (same mechanism the status LED uses).
            if (o instanceof THREE.Mesh && o.material instanceof THREE.MeshStandardMaterial && o.material !== goldMat && o.material !== gemMat) {
 ...
 * @param {THREE.MeshStandardMaterial | null} gemMat the category gem material
 * @param {THREE.Mesh | null} gemMesh the category gem mesh
 * @returns {THREE.MeshStandardMaterial} the steady status LED material (for focus flash)
 */
function buildSolderedChip(group, chipMat, goldMat, traceMat, busOffsetY, signalHex, gemMat, gemMesh) {
    ...
    // Polished gem seated in the chip's socket (top center, proud of the
    // body). Shares the steady driver below, so it breathes with the LED.
    if (gemMat && gemMesh) {
        gemMesh.position.set(0, 0, 0.155);
        group.add(gemMesh);
    }
    ...
    steadyLeds.push({ mat: ledMat, seed: Math.sin(group.position.x * 12.9898 + group.position.y * 78.233) * 43758.5453, base: 1.4, gem: gemMat });
    return ledMat;
}
...
function buildBreadboardPatch(group, signalHex, gemMat, gemMesh) {
    ...
    // Rough un-cut gem loose on the patch (slightly rotated — not seated,
    // like the micro chip). Shares the flicker driver below.
    if (gemMat && gemMesh) {
        gemMesh.position.set(0.1, 0.1, 0.085);
        group.add(gemMesh);
    }
    ...
    flickerLeds.push({ mat: ledMat, seed: flickerLeds.length * 2.4, gem: gemMat });
    return ledMat;
}
...
function applyChipFilter(chip) {
    ...
    if (dimmed) {
        ...
        if (chip.gemMat) dimmedLedMats.add(chip.gemMat);
    } else {
        dimmedLedMats.delete(chip.ledMat);
        if (chip.gemMat) dimmedLedMats.delete(chip.gemMat);
    }
...
export function updateProjectChips(elapsed) {
    ...
    if (motionPrefs.reduced) {
        for (const f of flickerLeds) {
            const v = ledValue(f.mat, 0.7);
            f.mat.emissiveIntensity = v;
            if (f.gem) f.gem.emissiveIntensity = ledValue(f.gem, GEM_BUILDING_EMISSIVE);
        }
        for (const s of steadyLeds) {
            const v = ledValue(s.mat, s.base);
            s.mat.emissiveIntensity = v;
            if (s.gem) s.gem.emissiveIntensity = ledValue(s.gem, GEM_SOLDERED_EMISSIVE);
        }
        return;
    }
    for (const f of flickerLeds) {
        if (dimmedLedMats.has(f.mat)) {
            ...
            if (f.gem) f.gem.emissiveIntensity = 0.05;
            continue;
        }
        const n = Math.sin(elapsed * 7 + f.seed) * Math.sin(elapsed * 13.7 + f.seed * 2);
        f.mat.emissiveIntensity = n > 0.55 ? 0.15 : 0.7 + n * 0.25;
        // The rough gem flickers with the LED — same value, same frame.
        if (f.gem) f.gem.emissiveIntensity = n > 0.55 ? 0.15 : GEM_BUILDING_EMISSIVE + n * 0.3;
    }
    ...
    for (const s of steadyLeds) {
        if (dimmedLedMats.has(s.mat)) {
            ...
            if (s.gem) s.gem.emissiveIntensity = 0.05;
            continue;
        }
        ...
        if (gsap.isTweening(s.mat) || (s.gem && gsap.isTweening(s.gem))) continue;
        const v = s.base + Math.sin(elapsed * 0.9 + s.seed) * 0.08;
        s.mat.emissiveIntensity = v;
        // The polished gem breathes with the shipped LED — same value.
        if (s.gem) s.gem.emissiveIntensity = GEM_SOLDERED_EMISSIVE + Math.sin(elapsed * 0.9 + s.seed) * 0.1;
    }
}
